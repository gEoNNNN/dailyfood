...
export default function MenuPage() {
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero} data-label={t.backgroundLabel}><span className={styles.kicker} data-reveal="left">{t.kicker}</span><h1 data-reveal="left" data-reveal-delay="1">{t.title}</h1><p data-reveal="up" data-reveal-delay="2">{t.intro}</p><div className={styles.heroStamp} data-reveal="scale" data-reveal-delay="3"><b>{menuCategories.length}</b><span>{t.stamp}</span></div></section>
      ...
      <footer className={styles.footer} data-reveal="up"><Link href="/"><Image src={logoImage} alt="Daily Kebab Burger" /></Link><p>{t.footer}</p><div><Link href="/delivery">{t.delivery}</Link><Link href="/privacy">{t.privacy}</Link><Link href="/">{t.back}</Link></div><small>© 2026 Daily Kebab Burger</small></footer>
    </main>
  );
}
